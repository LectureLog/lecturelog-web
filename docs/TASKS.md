...
- **Долг из B1 — fail-fast на пустом webhook-секрете.** `VerifyWebhookSignature`
  (`internal/coreclient/webhook.go`) — чистая функция: при пустом секрете она
  посчитает валидный HMAC по пустому ключу и может вернуть `true`. Поэтому при
  загрузке конфига C0-config ОБЯЗАН падать на старте, если
...
> проверяет подпись на `POST /tasks`, `POST /uploads`, `DELETE /tasks/{id}` —
> платформа исходящие НЕ подписывает (защита = сетевой контур + общие секреты).
> HMAC живёт ТОЛЬКО на вебхуке ядро→платформа: ядро подписывает callback
> (`X-Webhook-Signature` = HMAC-SHA256 от байтов тела), платформа его
> ВЕРИФИЦИРУЕТ (`VerifyWebhookSignature`). В ядро ничего интегрировать НЕ нужно —
> долг чисто документационный: переписать формулировки, чтобы следующая волна не
> реализовывала несуществующую «подпись исходящих».
...
- **Валидация контента на платформе ДО похода в ядро (из обсуждения B1).**
  Отсекать заведомо негодный вход на загрузке, чтобы не гонять мусор в ядро и
  не ждать его 400/422: расширение/MIME файла (поддерживаемые аудио/видео),
  размер, непустота; для YouTube-режима — валидность URL. Это входная гигиена,
  НЕ дублирование доменных enum'ов ядра (`media` и пр. валидирует ядро —
  источник правды, дублировать нельзя из-за риска рассинхрона). Место проверки —
  здесь (upload), НЕ в `coreclient` (тонкий транспорт; файл в него не попадает —
  в ядро уходит уже `s3_key`/`video_url`).
