...
C1-lecture ✅ смержен; C1-upload 🟡 5/7 под-атомов смержены. integration=`bbe2082` (локально,
НЕ запушен — push/PR в конце волны). Ворота зелёные, дерево чисто, worktree-ов нет (пауза
2026-06-24 на безопасной границе). Детали волны — `docs/HANDOFF-C1.md`.
...
ВОЛНА C1 НАЧАТА, идёт АТОМ C1-upload. Прочитай `docs/HANDOFF-C1.md` — там детальное состояние,
разбивка C1-upload на 7 под-атомов, контракты пакета upload, долги, грабли Codex.
- C1-lecture ✅ смержен (48f55b3, docs b452a96).
- C1-upload 🟡 5/7 под-атомов смержены (integration=`bbe2082`). Осталось 2: **upload-ui**
  (templ-форма, грабли детерминизма) и **upload-mount** (проводка в main, закрывает BLOCKER
  «routes не подключены»). НЕ запушено (push/PR — в конце волны).
ПЕРВОЕ ДЕЙСТВИЕ: продолжи C1-upload с под-атома **upload-mount** (рекомендация; легче, закрывает
BLOCKER), затем **upload-ui** — ИЛИ наоборот/∥ (UI и mount слабо связаны; mount монтирует
GET /upload-страницу из UI, если UI ещё нет — монтирует только API). Детали, контракты, долги,
порядок — в HANDOFF-C1.md разделы «C1-upload — разбивка» и «Контракты пакета internal/upload».
Объёмные атомы (reader) ДРОБИТЬ на под-атомы (правило владельца). hub/reader независимы — ∥ можно.
